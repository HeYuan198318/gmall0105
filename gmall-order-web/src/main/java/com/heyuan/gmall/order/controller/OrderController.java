package com.heyuan.gmall.order.controller;
...
public class OrderController {
    ...
    public ModelAndView submitOrder(String receiveAddressId, BigDecimal totalAmount, String tradeCode, HttpServletRequest request, HttpServletResponse response, HttpSession session) {
        ...
        if (success.equals("success")) {
            ...
            omsOrder.setCreateTime(new Date());//创建时间
            omsOrder.setDiscountAmount(null);//折扣后的价格
            //omsOrder.setFreightAmount(); 运费，支付后，在生成物流信息时
            omsOrder.setMemberId(memberId);
            omsOrder.setMemberUsername(nickname);
            omsOrder.setNote("快点发货");//订单备注
            String outTradeNo = "gmall";//商品订单号
            ...
            omsOrder.setPayAmount(totalAmount);//支付金额
            omsOrder.setOrderType(1);//订单类型1正常,秒杀订单
            //获取收货人对象
            ...
            Date time = c.getTime();
            omsOrder.setReceiveTime(time);//收货时间
            ...
            for (OmsCartItem omsCartItem : omsCartItems) {
                if (omsCartItem.getIsChecked().equals("1")) {
                    ...
                    omsOrderItem.setOrderSn(outTradeNo);// 外部订单号，用来和其他系统进行交互，防止重复(阿里支付)
